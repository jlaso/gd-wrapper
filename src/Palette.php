...
class Palette
{
    ...
    protected $color;
    /** @var bool|int */
    protected $internalPalette = false;

    /**
     * you can create a palette with the baseColor or with an integer between 0 and 199, see colours defined below
     * @param string|int $baseColor
     */
    public function __construct($baseColor)
    {
        if (is_int(intval($baseColor)) && (intval($baseColor) < count($this->coolPalettes))) {
            $this->internalPalette = intval($baseColor);
            $baseColor = ltrim($this->coolPalettes[$this->internalPalette][0], '#');
        }
        $this->color = new Color($baseColor);
        $this->baseColor = $this->color->getValue();
    }
    ...
    public function getColors($total = 3)
    {
        ...
        if (false !== $this->internalPalette) {
            ...
            $i = 0;
            $colors[$names[$i++]] = $this->color;
            $degrees = 360 / $total;
            while (--$total > 0) {
                $hsl = $this->color->getHsl();
                $hsl['H'] += $degrees;
                if ($hsl['H'] > 360) $hsl['H'] -= 360;
                $colors[$names[$i++]] = new Color($hsl);
                $degrees += $degrees;
            }
        }
        return $colors;
